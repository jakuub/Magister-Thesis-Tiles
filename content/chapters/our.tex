...
\section{Architecture}\label{sec:our-architecture}
  ...
  \subsection{Core}\label{subsec:our-architecture-core}
    ...
    \subsubsection{ComponentDescription}\label{subsubsec:our-architecture-core-component-description}
      ...
      \begin{itemize}
        ...
        \item \textbf{Key} \hfill \\
					...
					If component's function \texttt{render} returns list of children with keys, 
    ...
    \subsubsection{Node}\label{subsubsec:our-architecture-core-node}
      ...
      Method \texttt{List<NodeChange> update()} is executing the update process. 
      ...
      \begin{enumerate}
        ...
        \item if component of this node needs to update (\texttt{isDirty == true}), update this node with rearrangement of children,
				\item add results of update calls on children to result,\todo {changed}
    ...
    \subsubsection{NodeChange}\label{subsubsec:our-architecture-core-node-change}
      ...
      It is used to mirror changes in the virtual DOM with the real DOM. 
			...
			When some node in the virtual DOM is updated by method \texttt{update}, the list of changes is returned. \todo{changed}
  ...
  \subsection{Life-cycle}\label{subsec:our-architecture-lifecycle}
    ...
    When a "higher" node wants to update the node of the component, the component will first receive props, 
    then it is asked, if it should be updated, and if yes, then it is rendered.
    After that, it was updated, of course.
		\todo{rewrite - subsequenty...}
    ...
    \subsubsection{Did mount}\label{subsec:our-architecture-lifecycle-didmount}
      ...
      \texttt{Component} life-cycle \textbf{Did mount} event is implemented by method \texttt{didMount}. 
      It is called after component is mounted to the real DOM. 

      This is the correct place to initialize for example the timers, stream listeners etc. 

      For example, in our \texttt{MyTodoComponent} we should listen for change of the todo on the server, 
    ...
    \subsubsection{Will receive props}\label{subsec:our-architecture-lifecycle-willreceiveprops}
      ...
      It is called every time, when component will receive new \texttt{props}, 
